	...
	if( xSemaphoreTake(xBufferSemaphore, (TickType_t) 100) == pdTRUE ) {
		...
		if (item != NULL) {
			...
			if (item != NULL) {
				...
				char * tempBuffer = (char *) realloc(linesBuffer, initial_item_size + item_size);
				if(tempBuffer == NULL) {
					ESP_LOGE(LOG_TAG, "Memory allocation failed while pulling log lines");
					free(linesBuffer);
					linesBuffer = NULL;
				} else {
					linesBuffer = tempBuffer;
				}
